...
import { useForgotMutation, useGetQuestionsQuery, useQuestionVerifyMutation } from '../slices/userApiSlice';
            ...
            return (
                <div className="bg-black h-screen flex flex-col">
                    ...
                    <main className="container mx-auto flex justify-center items-center flex-grow">
                        ...
                        <SecurityReset email={email} navigate={navigate} setStep={setStep} />
...
function SecurityReset({ email, navigate, setStep }) {
    ...
    const submitHandler = async (e) => {
        e.preventDefault()
        try {
            const res = await questionVerify({q1: data.q1, q1a, q2: data.q2, q2a, q3:data.q3, q3a, email }).unwrap();
    ...
    return (
        ...
        : (
            ...
            <form onSubmit={submitHandler}>
                <div className="mb-4">
                    <label htmlFor="email" className="block text-gray-300 text-sm font-medium">{data.q1}</label>
                    <input
                        type="text"
                        className="mt-1 px-4 py-2 w-full border rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-gray-800 text-white"
                        placeholder="Answer"
                ...
                <div className="mb-4">
                    <label htmlFor="email" className="block text-gray-300 text-sm font-medium">{data.q2}</label>
                    <input
                        type="text"
                        className="mt-1 px-4 py-2 w-full border rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-gray-800 text-white"
                        placeholder="Answer"
                ...
                <div className="mb-4">
                    <label htmlFor="email" className="block text-gray-300 text-sm font-medium">{data.q3}</label>
                    <input
                        type="text"
                        className="mt-1 px-4 py-2 w-full border rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-gray-800 text-white"
                        placeholder="Answer"
...
function QuestionError({setStep}) {
    ...
    return (
        <div className="w-full max-w-md p-6 bg-gray-900 rounded-lg shadow-md">
            <h2 className="text-2xl font-semibold text-white mb-6">Unable to get Security Questions</h2>
            <form onSubmit={submitHandler}>
                <div className="mb-6">
                    <button
                        type="submit"
